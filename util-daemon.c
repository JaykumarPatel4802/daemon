        ...
        while ((entry = readdir(containerDir)) != NULL) {
            if (entry->d_type == DT_DIR && strcmp(entry->d_name, ".") != 0 && strcmp(entry->d_name, "..") != 0) {
                if (existingContainersCount < MAX_INITIAL_CONTAINERS) {
                    printf("Init Docker Container: %s\n", entry->d_name);
...
void processContainer(const char *containerName, sqlite3 *db) {
    ...
    // 5. Get energy utilization
    // Docker id is "containerName"
    // Can get docker pid using: <docker inspect -f '{{.State.Pid}}' <docker_id>>
    // run <sudo energat -pid <PID>>
...
int main() {
    ...
    printf("Opening invoker_data.db\n");
    ...
    printf("Creating table if it doesn't exist\n");
